...
Regular::Regular(Camera *c_ptr, ViewPlane *v_ptr, int n) : Sampler(c_ptr, v_ptr), n(n)
{
}

Regular::Regular() : Sampler(), n(1)
{
}
Regular::Regular(int n) : Sampler(), n(n)
{
}
...
std::vector<Ray> Regular::get_rays(int px, int py) const
{
  ...
  for (int p = 0; p < n; p++)
  {
    ...
    samplePoint.x = x_spacing * (px + (p + 0.5) / n);

    for (int q = 0; q < n; q++)
    {
      samplePoint.y = y_spacing * (py + (q + 0.5) / n);
