// Credit: Matthew Calligaro <matthewcalligaro@hotmail.com>

#include "../cameras/Parallel.hpp"
#include "../geometry/Plane.hpp"
#include "../geometry/Sphere.hpp"
#include "../geometry/Triangle.hpp"
#include "../materials/Phong.hpp"
#include "../materials/Reflective.hpp"
#include "../materials/Transparent.hpp"
#include "../samplers/Simple.hpp"
#include "../utilities/Constants.hpp"
#include "../lights/DirectionalLight.hpp"
#include "../world/World.hpp"
#include "../samplers/Regular.hpp"
#include "../tracers/ShadowTracer.hpp"
void World::build(void) {
  // View plane  .
  int n = 10;
  vplane.top_left.x = -n;
  vplane.top_left.y = n;
  vplane.top_left.z = 20;
  vplane.bottom_right.x = n;
  vplane.bottom_right.y = -n;
  vplane.bottom_right.z = 20;
  vplane.hres = 400;
  vplane.vres = 400;
  this->set_tracer(new ShadowTracer(3));


  // colors
  RGBColor yellow(1, 1, 0);  // yellow
  RGBColor brown(0.71, 0.40, 0.16);  // brown
  RGBColor darkGreen(0.0, 0.41, 0.41);  // darkGreen
  RGBColor orange(1, 0.75, 0);  // orange
  RGBColor green(0, 0.6, 0.3);  // green
  RGBColor lightGreen(0.65, 1, 0.30);  // light green
  RGBColor darkYellow(0.61, 0.61, 0);  // dark yellow
  RGBColor lightPurple(0.65, 0.3, 1);  // light purple
  RGBColor darkPurple(0.5, 0, 1);  // dark purple
  RGBColor grey(0.3, 0.35, 0.3);  // grey
  
  // Background color.
  bg_color = grey;
  
  // directional light
  DirectionalLight* light_ptr = new DirectionalLight(white, Vector3D(0.3, 0.3, -1));
  add_light(light_ptr);

  // Camera and sampler.
  set_camera(new Parallel(0, 0, -1));
  // sampler_ptr = new Simple(camera_ptr, &vplane);
  sampler_ptr = new Regular(camera_ptr, &vplane,4);
  
  // sphere
  Sphere* sphere_ptr = new Sphere(Point3D(0, 0, 5), 5); 
  sphere_ptr->set_material(new Transparent(lightPurple));
  add_geometry(sphere_ptr);

  for (int x = -8; x <= 8; x += 2) {
    for (int y = -8; y <= 8; y += 2) {
      Sphere* s = new Sphere(Point3D(x, y, -1), 1);
      s->set_material(new Phong((x + 8.0) / 16.0, 0, (y + 8.0) / 16.0));
      add_geometry(s);
    }
  }
}
