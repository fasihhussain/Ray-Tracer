...
void World::build(void) {
  // View plane  .
  int factor = 1;
  // std::cout << 9*9 * factor*factor << std::endl; 
  int n = factor*10;
  long n_pixels = 0;
  vplane.top_left.x = -n;
  vplane.top_left.y = n;
  ...
  for (int x = -8*factor; x <= 8*factor; x += 2) {
    for (int y = -8*factor; y <= 8*factor; y += 2) {
      Sphere* s = new Sphere(Point3D(x, y, 0), 1);
      s->set_material(new Cosine((x + 8.0) / (16.0*factor), 0, (y + 8.0) / (16.0*factor)));
      add_geometry(s);
      n_pixels++;
    }
  }
